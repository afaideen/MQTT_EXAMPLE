...
void APP_Tasks(void) {
    ...
    if(!SWITCH1_Get() || !SWITCH2_Get() || !SWITCH3_Get() || !SWITCH4_Get() )
    {
        if( SYS_TMR_TickCountGet() - tSw1 >= SYS_TMR_TickCounterFrequencyGet() * 0.25 )
        {
            ...
            {
//                strcpy(appData.urlBuffer, "http://192.168.137.66:5000/");//LINUX 
                strcpy(appData.urlBuffer, "http://192.168.137.1:5000/");//WIN
                while(!SWITCH3_Get());   
            }
            else if(!SWITCH4_Get())
            {
//                strcpy(appData.urlBuffer, "https://192.168.137.66:5000/");//LINUX
                strcpy(appData.urlBuffer, "https://192.168.137.1:5000/testing");//WIN
                while(!SWITCH4_Get());   
            }
